...
impl Pump {
    ...
    pub async fn build_swap_ixn_by_mint(
        ...
        let (amount, _amount_ui_pretty) = match swap_config.swap_direction {
            SwapDirection::Buy => {
            ...
            SwapDirection::Sell => {
                ...
                let amount = match swap_config.in_type {
                    SwapInType::Qty => {
                        ui_amount_to_amount(swap_config.amount_in, in_mint.decimals)
                    }
                    SwapInType::Pct => {
                        ...
                        if amount_in_pct == 1.0 {
                            ...
                            (amount_in_pct * 100.0) as u64 * in_account.amount / 100
                        }
                    }
                };
                (
        ...
        let (token_amount, sol_amount_threshold, input_accouts) = match swap_config.swap_direction {
            SwapDirection::Buy => {
                let max_sol_cost = max_amount_with_slippage(amount, slippage_bps);
                let amount_result = u128::from(amount)
                    .checked_mul(virtual_token_reserves)
                    .expect("Failed to multiply amount by virtual_token_reserves: overflow occurred.")
            ...
            SwapDirection::Sell => {
                let sol_output = u128::from(amount)
                .checked_mul(virtual_sol_reserves)
                .expect("Failed to multiply amount by virtual_sol_reserves: overflow occurred.")
    ...
    /// Get token pool information for bonding curve analysis
    pub async fn get_token_pool_info(&self, token_mint: &Pubkey) -> Result<PoolInfo> {
        let pump_program = Pubkey::from_str(PUMP_PROGRAM)?;
        
        // Get bonding curve info for the token
        let (_bonding_curve, _, bonding_curve_reserves) = 
            get_bonding_curve_account(self.rpc_client.clone().unwrap(), *token_mint, pump_program).await?;
            
        // Calculate liquidity and current price based on bonding curve formula
        let virtual_sol_reserves = bonding_curve_reserves.virtual_sol_reserves as f64 / 1_000_000_000.0; // Convert to SOL
        let virtual_token_reserves = bonding_curve_reserves.virtual_token_reserves as f64;
        
        // Current price is calculated as virtual_sol_reserves / virtual_token_reserves
        ...
        Ok(PoolInfo {
            ...
            liquidity: virtual_sol_reserves,
            price,
